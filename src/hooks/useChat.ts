...
import { MambaHelper } from '@/lib/custom-mamba-helper';
import { GeminiHelper } from '@/lib/gemini-helper';
...
export const useChat = ({ id, initialMessages }: UseChatOptions) => {
  ...
  const handleChatCompletion = async (
    ...
    // Check if we're using the custom Mamba model
    if (selectedModel.useCustomBackend) {
      const mambaHelper = new MambaHelper();
      const completion = mambaHelper.generateCompletion(
        userInput,
        handleChatCallbacks
      );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Skip engine initialization for custom models
      if (selectedModel.useCustomBackend) {
        if (selectedModel.isGemini) {
          // Use Gemini API
          const geminiHelper = new GeminiHelper();
          const completion = geminiHelper.generateCompletion(
            userMessage.content.toString(),
            id, // Pass the chat ID to maintain conversation history
            handleChatCallbacks
          );
